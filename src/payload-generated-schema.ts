...
export const main_menu_rels = pgTable(
  ...
  (columns) => [
    index('main_menu_rels_order_idx').on(columns.order),
    index('main_menu_rels_parent_idx').on(columns.parent),
    index('main_menu_rels_path_idx').on(columns.path),
    index('main_menu_rels_pages_id_idx').on(columns.pagesID),
    foreignKey({
      columns: [columns['parent']],
      foreignColumns: [main_menu.id],
